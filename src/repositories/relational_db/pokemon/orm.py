...
from repositories.relational_db import pokemon
...
class Type(Base):
    ...
    id: Mapped[UUIDStr] = mapped_column(
        String(32), primary_key=True, default=build_uuid4_str
    )
    name: Mapped[str] = mapped_column(String(256))
...
class PokemonEvolution(Base):
    ...
    previous_pokemon: Mapped[Optional["Pokemon"]] = relationship(
        "Pokemon",
        foreign_keys=[previous_no],
        back_populates="next_evolutions",
        lazy="raise",
    )
    next_pokemon: Mapped[Optional["Pokemon"]] = relationship(
        "Pokemon",
        foreign_keys=[next_no],
        back_populates="previous_evolutions",
        lazy="raise",
    )
