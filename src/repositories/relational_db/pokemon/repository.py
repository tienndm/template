...
class RelationalDBPokemonRepostory(AbstractPokemonRepository):
    ...
    async def get(self, no: PokemonNumberStr) -> PokemonModel:
        stmt = (
            ...
            .options(
                ...
                selectinload(Pokemon.next_evolutions).joinedload(
                    PokemonEvolution.next_pokemon
                ),
            )
        )
        ...
        return PokemonOrmMapper.ormToEntity(pokemon)
    ...
    async def list(self, params: Optional[GetPokemonParamsModel] = None) -> List[PokemonModel]:
        ...
        return list(map(PokemonOrmMapper.ormToEntity, pokemons))


    async def create(self, data: CreatePokemonModel) -> PokemonNumberStr:
        print(f"#repository.create - create - data: {data}")
